...
import { ChevronRight, ChevronLeft, Clock, MapPin, Check, Calendar, Phone, Minus, Plus } from 'lucide-react';
...
const SERVICES = [
  ...
  },
  {
    id: 'party',
    name: 'Makeup Party',
    description: 'Séance en groupe — anniversaire, EVJF ou soirée entre amies.',
    duration: '1h',
    price: 160,
    accent: '#FEF3C7',
...
function fmtDate(d: Date)     { return `${DAY_SHORT[d.getDay()]} ${d.getDate()} ${MONTHS[d.getMonth()]}`; }
function fmtDateLong(d: Date) { return `${d.getDate()} ${MONTHS[d.getMonth()]} ${d.getFullYear()}`; }
        ...
        <div className="max-w-sm w-full">
          <div className="bg-white border border-gray-100 overflow-hidden">
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-gray-400 text-xs mb-8">Un SMS de confirmation vous sera envoyé.</p>
              <div className="space-y-4 text-left">
                {[
                  { label: 'Service',    value: service.name },
                  { label: 'Personnes', value: `${quantity} personne${quantity > 1 ? 's' : ''}` },
  ...
  return (
    <main className="bg-gray-50 min-h-screen">
      <SEO
        ...
        description="Réservez votre séance maquillage dans une boutique MAC. Maquillage de jour, nuit, mariée, photoshoot. Remboursable en produit."
        ...
        <div className="max-w-2xl mx-auto flex">
          {([
            ...
            <button
              ...
              className={`flex-1 py-3.5 text-[10px] font-black tracking-widest uppercase border-b-2 transition-all flex items-center justify-center gap-1.5 ${
                step === n
            ...
            >
              <span className={`w-4 h-4 rounded-full text-[9px] flex items-center justify-center flex-shrink-0 font-black transition-colors ${
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-4">
        ...
        {step === 1 && (
          <>
              ...
              {SERVICES.map((s) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white cursor-pointer border-2 transition-all overflow-hidden ${
                      isSelected ? 'border-black' : 'border-transparent shadow-sm hover:shadow-md hover:border-gray-100'
                    }`}
                  >
                    {/* Top accent bar */}
                    ...
                    <div className="p-5">
                      <div className="flex items-center gap-5">
                        {/* Circle swatch */}
                        <div
                          className="w-14 h-14 rounded-full flex-shrink-0 transition-transform"
                          style={{ backgroundColor: s.accent }}
                        />

                        {/* Text */}
                        <div className="flex-1 min-w-0">
                          <p className="font-black text-base tracking-wide leading-tight">{s.name}</p>
                          <p className="text-sm text-gray-500 mt-1 leading-snug">{s.description}</p>
                        ...
                        <div className="flex flex-col items-end gap-2 flex-shrink-0 ml-2">
                          <p className="font-black text-xl leading-none">{s.price} MAD</p>
                          {'priceNote' in s && s.priceNote && (
                            <p className="text-xs text-gray-400">{s.priceNote}</p>
                          )}
                      ...
                      {isSelected && (
                        ...
                        >
                          <div>
                            <p className="text-xs font-black uppercase tracking-widest">Nombre de personnes</p>
                            <p className="text-[11px] text-gray-400 mt-0.5">
                              {quantity > 1
                                ? `Total : ${s.price * quantity} MAD`
                                : 'Chaque personne sera maquillée'}
                            </p>
                          </div>
                          <div className="flex items-center gap-3">
                            <button
                              ...
                              disabled={quantity <= 1}
                              className="w-9 h-9 border-2 border-black flex items-center justify-center hover:bg-black hover:text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                            >
                              <Minus size={14} strokeWidth={2.5} />
                            </button>
                            <span className="text-xl font-black w-6 text-center tabular-nums">{quantity}</span>
                            <button
                              type="button"
            ...
            <button
              ...
              disabled={!canStep2}
              onClick={() => setStep(2)}
              className="w-full bg-black text-white py-4 text-xs font-black tracking-widest uppercase flex items-center justify-center gap-2 hover:bg-gray-800 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
        ...
        {step === 2 && (
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {STORES.map((s) => (
                  <button
                    ...
                    className={`text-left bg-white border-2 p-5 transition-all ${
                      store === s.id ? 'border-black' : 'border-transparent shadow-sm hover:border-gray-200'
                    }`}
                  >
                    <div className={`w-2 h-2 rounded-full mb-3 transition-colors ${store === s.id ? 'bg-black' : 'bg-gray-200'}`} />
            ...
            <div>
                ...
                {days.map((d, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-shrink-0 w-[3.75rem] py-3.5 flex flex-col items-center gap-0.5 border-2 transition-all ${
                        isSel
                    ...
                    >
                      ...
                      <span className={`text-[9px] ${isSel ? 'text-gray-400' : 'text-gray-300'}`}>{MONTHS[d.getMonth()]}</span>
                      {isSatDay && (
                        <span className={`text-[8px] font-bold mt-0.5 ${isSel ? 'text-gray-400' : 'text-amber-500'}`}>max {MAX_SAT}</span>
                      )}
                    </button>
                  );
                })}
              ...
              {satFull && (
                <p className="text-xs text-red-600 font-bold mt-2">Ce samedi est complet — max {MAX_SAT} rendez-vous par samedi.</p>
              )}
            </div>
            ...
            {selectedDay && !satFull && (
              <div>
                <p className="text-[10px] font-black tracking-widest uppercase text-gray-400 mb-3 flex items-center gap-2">
                  <Clock size={11} /> Heure
                  <span className="text-gray-300 font-normal normal-case tracking-normal">· à partir de 12h00</span>
                </p>
        ...
        {step === 3 && (
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Inline booking summary — no separate sidebar */}
            {service && storeInfo && selectedDay && time && (
              <div className="bg-white border-2 border-black p-5">
                <div className="h-1 -mt-5 -mx-5 mb-5" style={{ backgroundColor: service.accent }} />
                <p className="text-[10px] font-black tracking-widest uppercase text-gray-400 mb-4">Récapitulatif</p>
                <div className="grid grid-cols-2 gap-y-3 text-sm">
                  <span className="text-gray-500">Service</span>
                  <span className="font-bold text-right">{service.name}</span>
                  <span className="text-gray-500">Personnes</span>
                  <span className="font-bold text-right">{quantity}</span>
                  <span className="text-gray-500">Boutique</span>
                  <span className="font-bold text-right">MAC {storeInfo.name}</span>
                  <span className="text-gray-500">Date</span>
                  <span className="font-bold text-right">{fmtDate(selectedDay)} à {time}</span>
                  <span className="font-black pt-3 border-t border-gray-100">Total</span>
                  <span className="font-black text-right pt-3 border-t border-gray-100">{total} MAD</span>
                </div>
              </div>
            )}
